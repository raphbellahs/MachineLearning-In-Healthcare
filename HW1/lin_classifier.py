    ...
    for c in C:
        for p in penalty:
            ...
            for train_idx, val_idx in kf.split(X, y):  # val=validation
                ...
                features = ('LB','ASTV')
                nsd_x = scaler.fit_transform(x_train,selected_feat=features,mode=mode,flag=False)
                nsd_val = scaler.fit_transform(x_val,selected_feat=features,mode=mode,flag=False)
                y_pred,_ = pred_log(logreg,nsd_x,y_train,nsd_val,flag=True)
                loss_val_vec[k]=log_loss(y_val,y_pred)
                k = k+1
            validation_list.append({'C':c,'penalty':p,'mu':np.mean(loss_val_vec),'sigma':np.std(loss_val_vec)})
...
def odds_ratio(w, X, selected_feat='LB'):
    ...
    ''' odd = p(Normal)/1-p(Normal) '''
    
    normal_label = w[0,:]
    odds = np.exp(X@np.transpose(normal_label)).median()
    feature_vec = X.columns.get_loc(selected_feat)
    odd_ratio = np.exp(normal_label[feature_vec])
